...
public class PlayerScript : MonoBehaviour {
    ...
    void Dash(int direction) {
        ...
        rb.gravityScale = 0f;
        rb.velocity = new Vector2(rb.velocityX, 0f);

        isDashing = true;
        knockbackTimer = 0f; // Cancel any knockback
        rb.velocity = new Vector2(direction * dashSpeed * speedMultiplier, rb.velocityY);
    ...
    void HandlePlayerCollision(Collision2D collision) {
        ...
        // If this player is dashing
            ...
            isDashing = false;
            rb.velocity = new Vector2(rb.velocityX * 0.3f, rb.velocityY); // Slow down 
        }
    }
    #endregion
    ...
    void UpdateAnimation() {
        ...
        if (Input.GetKey(leftKey) || Input.GetKey(rightKey)) {
            inputSpeed = Mathf.Abs(rb.velocityX) / moveSpeed;
        }
    ...
    void MovementInput() {
        ...
        if (knockbackTimer <= 0 && !isDashing) {
            ...
            rb.velocity = new Vector2(inputX * currentMoveSpeed, rb.velocityY);
        }
    }
    #endregion
}
